<template>
  <MainLayout
    active-nav="logs"
    user-name="管理员"
    active-sub-nav="communication-logs"
    @nav-change="handleNavigation"
  >
    <div class="content-area">
      <!-- 面包屑导航 -->
      <PageHeader title="通信日志" :breadcrumbs="breadcrumbs" :actions="pageActions" />
      ...
      <div class="log-viewer-container">
        <div class="log-list" ref="logContainer" @scroll="handleScroll">
          ...
          <div v-if="filteredLogs.length === 0 && !isLoading" class="no-logs">
            ...
            {{ allLogs.length === 0 ? '暂无通信日志数据' : '没有匹配的通信日志' }}
          </div>

          <!-- 加载中 -->
          <div v-if="isLoading" class="loading-indicator">
            <i class="fas fa-spinner fa-spin"></i> 正在加载通信日志...
          </div>
  ...
  data() {
    return {
      breadcrumbs: [
        ...
        { title: '通信日志', link: '/system/communication-logs' },
      ],
      pageActions: [
        {
          text: '导出通信日志',
          ...
          handler: this.exportCommunicationLogsWithAxios,
        },
      ],
  ...
  methods: {
    ...
    async loadMoreLogs() {
      ...
      try {
        ...
        console.error('加载更多通信日志失败:', error)
      } finally {
    ...
    startAutoRefresh() {
      ...
      this.autoRefreshTimer = setInterval(async () => {
        ...
        try {
          ...
          console.error('自动刷新通信日志失败:', error)
        }
      }, 3000)
    },
    ...
    async exportCommunicationLogsWithAxios() {
      try {
        console.log('正在导出通信日志...')
        ...
        let filename = 'communication_logs.zip'
        ...
        console.log('通信日志导出成功')
      } catch (error) {
        console.error('导出通信日志失败:', error)
      }
    },
  },
}
